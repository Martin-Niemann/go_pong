...
type GameplayMultiplayer struct {
	ball Ball
	p1   Paddle
	p2   Paddle
}

func (g *GameplayMultiplayer) initialize_gameobjects() {
	g.ball = Ball{
		x:               screen_width / 2,
		y:               screen_height / 2,
		speed_x:         ball_speed,
...
func (g *GameplayMultiplayer) host_update(player_paddle *Paddle) {
	player_paddle.poll_input()
	g.p1.update()
	g.p2.update()
	g.ball.update()

	g.ball.check_collission(&g.p1)
	g.ball.check_collission(&g.p2)

	/*fmt.Print("\033[H\033[2J")
	fmt.Println("Ball x: ", g.ball.x)
	fmt.Println("Ball y: ", g.ball.y)
	fmt.Println("P1 y: ", g.p1.y)
	fmt.Println("P2 y: ", g.p2.y)*/
}

func (g *GameplayMultiplayer) client_update(player_paddle *Paddle) {
	player_paddle.poll_input()
...
func (g *GameplayMultiplayer) draw() {
	rl.BeginDrawing()
